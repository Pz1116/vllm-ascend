...
"""
PD-separated pure producer (P node) async KV load patch.

vLLM's Scheduler only enters the WAITING queue admission loop while
``token_budget > 0``. During PD disaggregation with an async remote KV load
(AscendStoreConnector load_async=true), the producer node can spend steps
with zero local compute budget while requests are still waiting for remote KV.
This blocks async KV recv admission, leaving the system in empty scheduling
steps.

This patch only activates on a PD-separated pure producer node
...
class PDProducerAsyncKVScheduler(Scheduler):
    """Scheduler variant that lets async KV loads use otherwise-empty steps.

    The behavior delta is gated by ``pd_sep_async_kv_overrides``, which is
    only True on a PD-separated pure producer (kv_role="kv_producer" and
    kv_consumer=False).
    """
    ...
    def __init__(self, *args, **kwargs) -> None:
        ...
        self.pd_sep_async_kv_overrides = (
        ...
        if not preempted_reqs and self._pause_state == PauseState.UNPAUSED:
            ...
            while (self.waiting or self.skipped_waiting) and (
                ...
                if load_kv_async:
                    ...
                    num_new_tokens = 0
                elif self.pd_sep_async_kv_overrides and token_budget == 0:
    ...
    def _has_pending_waiting_async_load_candidate(self) -> bool:
        """True if a WAITING request might need a connector async KV load.

        Only enabled on PD-separated pure producer (P node).
        """
        if not self.pd_sep_async_kv_overrides:
            return False
...
# ``vllm.v1.core.sched.scheduler``. Replacing the module-global name with this
# subclass keeps every path that does not set scheduler_cls on the patched
# behavior; non-producer/non-PD configs diverge only in the guarded branches
# above and therefore behave exactly like upstream.
_sched_mod.Scheduler = PDProducerAsyncKVScheduler
